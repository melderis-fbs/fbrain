...
**Este archivo es el prompt de sistema base de todos los motores.** Se concatena antes del prompt específico de cada uno. No se edita por caso, no se ajusta por cliente, no se acorta para ahorrar tokens. Es lo que hace que dos consultoras distintas obtengan el mismo criterio.

Versionar en el repo. Cada corrida guarda el hash de este archivo en `diagnosticos.prompt_version`. Sin eso, el set de evaluación no significa nada.
